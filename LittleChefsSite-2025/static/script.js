...
submitItemBtn.onclick = async () => {
    addItemError.textContent = '';
    const val = itemInput.value;
    const parts = val.split('!!386!!');
    if (parts.length !== 3) {
        addItemError.textContent = 'Format: Name!!386!!Price!!386!!Description';
        return;
    }
    const [name, price, description] = parts;
    if (!name || isNaN(price) || !description) {
        addItemError.textContent = 'Invalid input.';
        return;
    }
    const res = await fetch('/admin/add_item', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (res.ok) {
        ...
        addItemError.textContent = 'Failed to add item.';
    }
};